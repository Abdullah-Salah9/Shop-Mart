...
export default async function CatDetails({params}: {params:Params}) {
    const {catId} = await params
    
    const catResponse = await fetch('https://ecommerce.routemisr.com/api/v1/categories/'+catId);
    const {data: category} : {data: CategoryI} = await catResponse.json();
    

    const proResponse = await fetch(`https://ecommerce.routemisr.com/api/v1/products?category=${catId}`)
    const {data: products} : {data: ProductI[]} = await proResponse.json()

    

    return <>
    <h2 className='text-xl font-bold pl-3 border-l-4 border-blue-500 mb-5'>{category.name}</h2>
